...
trait HasRocketTiles extends HasTiles
  ...
  // <local Int node, tile param, crossing param>
  private val crossingTuples = localIntNodes.zip(tileParams).zip(crossings).zipWithIndex
  val tiles: Seq[BaseTile] = crossingTuples.map { case (((lip, tp), crossing), i) =>
