...
abstract class BaseCoreplexModule[+L <: BaseCoreplex, +B <: BaseCoreplexBundle](
  ...
  val uselabel = p(UseLabel)
  val cachedPorts = if (p(HasTrafficGenerator) && uselabel) {
  ...
  val uncachedPorts = if (p(HasTrafficGenerator) && uselabel) {
  ...
  val controlledCachedPorts = (cachedPorts.zipWithIndex zip cachedControlCrossing) map {
    case ((p, i), cross) =>
    cross.io.in <> p
    cross.io.enable := (if (uselabel) buckets(i).io.enable else Bool(true))
    cross.io.out
  }

  val controlledUncachedPorts = (uncachedPorts.zipWithIndex zip uncachedControlCrossing) map {
    case ((p, i), cross) =>
    cross.io.in <> p
    cross.io.enable := (if (uselabel) buckets(i).io.enable else Bool(true))
    cross.io.out
